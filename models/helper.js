...
const callRoleStatus = async(role1,role2='') => {
  
    try {
        const client  = await pool.connect()
        let query = `select id from tbl_role where role_name=$1 or role_name=$2`
        let con_result = await client.query(query,[role1,role2])
